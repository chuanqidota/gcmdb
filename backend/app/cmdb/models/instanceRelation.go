...
type InstanceRelation struct {
	...
	SourceInstanceId uint `gorm:"column:source_id;type:uint;not null;uniqueIndex:idx_instance_relation_unique;comment:源实例id" json:"source_instance_id" binding:"required"`
	TargetInstanceId uint `gorm:"column:target_id;type:uint;not null;uniqueIndex:idx_instance_relation_unique;comment:目标实例id" json:"target_instance_id" binding:"required"`
}
