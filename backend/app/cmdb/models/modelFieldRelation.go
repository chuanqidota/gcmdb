...
type ModelFieldRelation struct {
	...
	SourceFieldId uint `gorm:"column:source_field_id;type:uint;not null;comment:源字段id" json:"source_field_id" binding:"required"`
	TargetFieldId uint `gorm:"column:target_field_id;type:uint;not null;comment:目标字段id" json:"target_field_id" binding:"required"`
}
