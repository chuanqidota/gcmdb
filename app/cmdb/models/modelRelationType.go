...
type ModelRelationType struct {
	...
	S2T  string `gorm:"column:s2t;type:string;size:255;not null;comment:源到目标的描述" json:"s2t" binding:"required"`
	T2S  string `gorm:"column:t2s;type:string;size:255;not null;comment:目标到源的描述" json:"t2s" binding:"required"`
}
